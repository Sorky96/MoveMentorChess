...
internal sealed class PlayerProfileSnapshotLoader
{
    ...
    public List<PlayerProfileSnapshot> Load(string? filterText, int limit)
    {
    ...
    private static PlayerProfileSnapshot? CreateSnapshotFromResult(GameAnalysisResult result)
    {
        ...
        string gameFingerprint = GameFingerprint.Compute(result.Game.PgnText);
        IReadOnlyList<StoredMoveAnalysis> moves = StoredMoveAnalysisMapper
            .FromAnalysisResult(
                new GameAnalysisCacheKey(gameFingerprint, result.AnalyzedSide, 0, 0, null),
                result,
                DateTime.MinValue)
            .ToList();
